...
import evince as ev
import sympy as sp
from scipy.interpolate import splint,LinearNDInterpolator, RegularGridInterpolator
        ...
        try:
            ...
            for i in range(1, 100):
                ...
                if i > 50:
                    ...
                    mean_estimate = (
                        mean_estimate * n_tot
                        + np.sum(x0.T * n2 * fn(*x0.T).real ** 2 * P**-1, axis=-1)
                    ) / (n_tot + n20)
                    n_tot += n20
                xn = x_
            ...
            n2 = np.mean(fn(*x0.T).real ** 2 * P**-1, axis=-1) ** -1
            x_ = (
                np.mean(x0.T**2 * n2 * fn(*x0.T).real ** 2 * P**-1, axis=-1)
                - xn.T**2
            )
...
def show(*p, t=None)->None:
    ...
    for i in list(p):
        ...
        if type(spe) in [np.array, list, np.ndarray]:
            ...
            plt.text(vec_R2[0] + 0.1, vec_R2[1], "%s" % i.__name__)
            ...
            if nd == 3:
                ...
                orb_pos = k3d.volume(
                    ...
                    opacity_function=np.linspace(0, 1, 30)[::-1] ** 0.2,
                )
...
def view(*p, t=None)->None:
    ...
    try:
        ...
        for i in list(p):
            ...
            plt.plot([vec_R2[0]], [vec_R2[1]], "o", color=(0, 0, 0))
            plt.text(vec_R2[0] + 0.2, vec_R2[1], "%s" % i.__name__)
        ...
        for shell in atom:
            for contracted in shell:
                ...
                for m in range(-l, l + 1):
                    bf = w[0] * get_solid_harmonic_gaussian(
                        a[0], l, m, position=[0, 0, 0]
                    )
                    for weights in range(1, len(w)):
                        bf += w[i] * get_solid_harmonic_gaussian(
                            a[i], l, m, position=[0, 0, 0]
                        )
...
class basisfunction:
    ...
    def __init__(self, sympy_expression, position=None, domain=None, name="\chi"):
        ...
        assert len(self.position) == self.dimension, (
            "Basis function position contains incorrect number of dimensions (%.i)."
            % self.dimension
        )

        # sympy expressions
        self.ket_sympy_expression = translate_sympy_expression(
            sympy_expression, self.position
        )
        self.bra_sympy_expression = translate_sympy_expression(
            sp.conjugate(sympy_expression), self.position
        )
        ...
        self.ket_numeric_expression = sp.lambdify(
            symbols, self.ket_sympy_expression, "numpy"
        )
        self.bra_numeric_expression = sp.lambdify(
            symbols, self.bra_sympy_expression, "numpy"
        )
            ...
            if self.domain is not None:
                ...
                for i in range(len(self.domain)):
                    domain.append(
                        [
                            np.array(
                                [self.domain[i].min(), other.domain[i].min()]
                            ).max(),
                            np.array(
                                [self.domain[i].max(), other.domain[i].max()]
                            ).min(),
                        ]
                    )
    ...
    def __mul__(self, other):
        ...
        return basisfunction(
            self.ket_sympy_expression * other.ket_sympy_expression,
...
class translation(operator):
    ...
    def __mul__(self, other):
        ...
        new_expression = translate_sympy_expression(
            other.get_ket_sympy_expression(), self.translation_vector
        )
    ...
    def __mul__(self, other):
        ...
        if self.variables is None:
            ...
            for i in range(len(symbols)):
                new_expression += sp.diff(
                    other.get_ket_sympy_expression(), symbols[i], self.order[i]
                )
        else:
            for i in range(len(self.variables)):
                new_expression += sp.diff(
                    other.get_ket_sympy_expression(), self.variables[i], self.order[i]
                )
        bs = basisfunction(new_expression)
        # bs.position = other.position
...
def translate_sympy_expression(sympy_expression, translation_vector):
    ...
    assert len(shifts) == len(
        translation_vector
    ), "Incorrect length of translation vector"

    return_expression = sympy_expression * 1

    for i in range(len(shifts)):
        if np.abs(translation_vector[i]) >= 20.0:
            return_expression = return_expression.subs(
                shifts[i], shifts[i] - sp.UnevaluatedExpr(translation_vector[i])
            )
        else:
            return_expression = return_expression.subs(
                shifts[i], shifts[i] - translation_vector[i]
            )
...
class kinetic_operator(operator):
    ...
    def __mul__(self, other):
        ...
        return ket([-0.5 * i for i in new_coefficients], basis=new_basis)
...
def get_onebody_coulomb_operator(
    position=np.array([0, 0, 0.0]), Z=1.0, p=None, variables=None
):
    return operator_expression(
        onebody_coulomb_operator(position, Z=Z, p=p, variables=variables)
    )
...
class onebody_coulomb_operator(operator):
    def __init__(
        self, position=np.array([0.0, 0.0, 0.0]), Z=1.0, p=None, variables=None
    ):
    ...
    def __mul__(self, other, r=None):
        ...
        for i in other.basis:
            new_basis.append(
                basisfunction(self.r_inv * i.ket_sympy_expression)
            )  # , position = i.position+self.position))
    ...
    def _repr_html_(self):
        if self.position is None:
            ...
            return "$ -\\frac{1}{\\vert \\mathbf{r} - (%f, %f, %f) \\vert }$" % (
                self.position[0],
                self.position[1],
                self.position[2],
            )
    ...
    def __mul__(self, other):
        ...
        for i in range(len(other.basis)):
            ...
            new_basis += (
                ...
                * other.basis[i].ket_sympy_expression
                / twobody_denominator(self.p1, self.p2, self.ndim)
            )
...
class twobody_coulomb_operator_old:
    ...
    def __mul__(self, other, r=None):
        ...
        for i in other.basis:
            new_basis.append(
                basisfunction(r_inv * i.ket_sympy_expression, position=i.position)
            )
...
class ket(object):
    ...
    def __mul__(self, other):
        if type(other) is ket:
            ...
            if str(type(other)).split("'")[1].split(".")[0] == "sympy":
                ...
                for i in range(len(self.basis)):
                    new_basis.append(
                        basisfunction(other * self.basis[i].ket_sympy_expression)
                    )
                ...
                return ket(
                    [other * i for i in self.coefficients],
                    basis=self.basis,
                    energy=self.energy,
                )
    ...
    def __rmul__(self, other):
        if str(type(other)).split("'")[1].split(".")[0] == "sympy":
            ...
            for i in range(len(self.basis)):
                new_basis.append(
                    basisfunction(self.basis[i].ket_sympy_expression * other)
                )
                new_coefficients.append(self.coefficients[i])
                new_energies.append(self.energy[i])
            return ket(new_coefficients, basis=new_basis, energy=new_energies)
    ...
    def __matmul__(self, other):
        ...
        if type(other) is ket:
            if self.bra_state:
                ...
                metric = np.zeros((len(self.basis), len(other.basis)), dtype=complex)

                for i in range(len(self.basis)):
                    for j in range(len(other.basis)):
                        if (
                            type(self.basis[i]) is np.ndarray
                            and type(other.basis[j]) is np.ndarray
                        ):
                            ...
                            if type(self.basis[i]) is basisfunction:
                                if type(other.basis[j]) is basisfunction:
                                    ...
                                    metric[i, j] = inner_product(
                                        self.basis[i], other.basis[j]
                                    )

                                if other.basis[j] is ket:
                                    # (basisfunction | ket )
                                    metric[i, j] = (
                                        ket([1.0], basis=[self.basis[i]]).bra
                                        @ other.basis[j]
                                    )
                            else:
                                if type(other.basis[j]) is basisfunction:
                                    # ( ket | basisfunction )
                                    metric[i, j] = self.basis[i].bra @ ket(
                                        [1.0], basis=[other.basis[j]]
                                    )
                ...
                if np.linalg.norm(metric.imag) <= 1e-10:
                    metric = metric.real
                return np.array(self.coefficients).T.dot(
                    metric.dot(np.array(other.coefficients))
                )
                    ...
                    if other.bra_state:
                        ...
                        for i in range(len(self.basis)):
                            for j in range(len(other.basis)):
                                ...
                                bij, sep = relabel_direct(
                                    self.basis[i].ket_sympy_expression,
                                    other.basis[j].ket_sympy_expression,
                                )
                                # bij = ket(bij)
                                bij = basisfunction(
                                    bij
                                )  # , position = other.basis[j].position)
                                bij.position = np.append(
                                    self.basis[i].position, other.basis[j].position
                                )
            ...
            if i not in found:
                ...
                for j in range(i + 1, len(self.basis)):
                    if type(self.basis[i]) is np.ndarray:
                        ...
                        if (
                            self.basis[i].ket_sympy_expression
                            == self.basis[j].ket_sympy_expression
                        ):
    ...
    def get_ccode(self):
        ...
        for i in range(len(self.coefficients)):
            if type(self.basis[i]) in [basisfunction, ket]:
                ...
                simp_ret = (
                    ret_i.subs(get_r2_sp(ret_i), sp.symbols("q"))
                    .simplify()
                    .subs(sp.pi, np.pi)
                )
    ...
    def get_bra_sympy_expression(self):
        ...
        for i in range(len(self.coefficients)):
            if type(self.basis[i]) in [basisfunction, ket]:
                ret += (
                    np.conjugate(self.coefficients[i])
                    * self.basis[i].bra_sympy_expression
                )
            ...
            if self.bra_state:
                for i in range(len(self.basis)):
                    result += np.conjugate(
                        self.coefficients[i]
                        * self.basis[i](*R)
                        * np.exp(-complex(0, 1) * self.energy[i] * t)
                    )
            else:
                for i in range(len(self.basis)):
                    result += (
    ...
    def measure(self, observable=None, repetitions=1):
        ...
        if observable is None:
            ...
            nd = len(symbols)
            sig = 0.1  # variance of initial distribution

            r = np.random.multivariate_normal(
                np.zeros(nd), sig * np.eye(nd), repetitions
            ).T
            # Metropolis-Hastings
            for i in range(1000):
                dr = np.random.multivariate_normal(
                    np.zeros(nd), 0.01 * sig * np.eye(nd), repetitions
                ).T
    ...
    def view(self, web=False, squared=False, n_concentric=100):
        ...
        if nd > 2:
            blend_factor = 0.1
        if web:
            self.m = ev.BraketView(
                self,
                additive=False,
                bg_color=[1.0, 1.0, 1.0],
                blender="    gl_FragColor = vec4(.9*csR - csI,  .9*abs(csR) + csI, -1.0*csR - csI, %f)"
                % blend_factor,
                squared=squared,
            ...
            self.m = ev.BraketView(
                ...
                blender="    gl_FragColor = vec4(.9*csR ,  csI, -1.0*csR, %f)"
                % blend_factor,
                squared=squared,
                n_concentric=n_concentric,
            )
...
def discrete_metropolis_hastings(
    P, n_samples=10000, n_iterations=100000, stepsize=None, T=0.001
):
    """
...
def relabel_direct(s1, s2):
    ...
    for i in p2:
        if i in p1:
            s2 = substitute_sequence(
                s2, get_default_variables(i), get_default_variables(p_max)
            )
            p_max += 1
...
class outerprod(object):
    ...
    def _repr_html_(self):
        return "$$\\sum_{ij} \\vert %s_i \\rangle \\langle %s_j \\vert$$" % (
            self.ket.__name__,
            self.bra.__name__,
        )
    ...
    def __mul__(self, other):
        if type(other) is ket:
            ...
            return ket(
                coefficients,
                basis=copy.copy(self.ket.basis),
                energy=copy.copy(self.ket.energy),
            )
...
class projector(object):
    ...
    def _repr_html_(self):
        return "$$\\sum_{i} \\vert %s_i \\rangle \\langle %s_i \\vert$$" % (
            self.ket.__name__,
            self.bra.__name__,
        )
...
def inner_product(b1, b2, operator=None, n_samples=int(1e6), grid=101, sigma=None):
    ...
    integrand = lambda *R, f1=b1.bra_numeric_expression, f2=b2.ket_numeric_expression, ri=ri, rj=rj: f1(
        *np.array([R[i] - ri[i] for i in range(len(ri))])
    ) * f2(
        *np.array([R[i] - rj[i] for i in range(len(rj))])
    )
    ...
    if len(variables_b1) == 1 and len(variables_b2) == 1:
        ...
        return onebody(
            integrand, np.ones(len(R)) * sigma, R, n_samples
        )  # , control_variate = "spline", grid = grid)
    """
    else:
...
def compose_basis(p):
    ...
    for charge in np.arange(p.charges.shape[0]):
        ...
        atom = np.argwhere(p.atomic_numbers == atomic_number)[
            0, 0
...
def get_control_variate(integrand, loc, a=0.6, tmin=1e-5, extent=6, grid=101):
    """
    ...
    return (
        RegularGridInterpolator(
            R_ - loc[:, None], data, bounds_error=False, fill_value=0
        ),
        I0,
        t,
    )
...
def onebody(
    integrand, sigma, loc, n_samples, control_variate=lambda *r: 0, grid=101, I0=0
):
    """
    ...
    if control_variate == "spline":
        control_variate, I0, t = get_control_variate(
            integrand, loc, a=0.6, tmin=1e-5, extent=6, grid=grid
        )
...
def eri_mci(
    ...
    integrand = (
        lambda *R, phi_p=phi_p, phi_q=phi_q, phi_r=phi_r, phi_s=phi_s, rp=pp, rq=pq, rr=pq, rs=ps: phi_p(
            R[0] - rp[0], R[1] - rp[1], R[2] - rp[2]
        )
    ...
    return (
        np.mean(
            (
                phi_p(u1[:, 0] - pp[0], u1[:, 1] - pp[1], u1[:, 2] - pp[2])
